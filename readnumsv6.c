// readnumsv6.c – Lab 01 – Dylan, Smith
...
// val is the data value in a node that needs to be deleted
int delete_node(int val)
{
	...
	else
	{
		...
		while (ptemp != NULL)
		{
			int nodeval = ptemp->datavalue;
			if (nodeval == val)
			{
				// node with delete value was found, delete this node
		...
		// no more nodes in the list, node to delete wasn't found
		printf("Could not find node with value %d to delete.\n", val);
	}
}
